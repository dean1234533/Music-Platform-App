  ...
  return (
    <section>
      ...
      <div className="rounded-xl border border-surface-border bg-surface-1 px-4 py-3">
        {isStandalone ? (
          <p className="text-sm text-support-400">Wavelength is installed on this device.</p>
        ) : canInstall ? (
          <Button size="sm" variant="secondary" onClick={() => void install()}>
            Install Wavelength
          </Button>
        ) : isIOS ? (
          <p className="text-sm text-ink-2">Tap Share, then “Add to Home Screen” to install Wavelength.</p>
        ) : (
