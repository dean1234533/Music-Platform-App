...
export function MobileNav({ items, moreExclude = [] }: { items: NavItem[]; moreExclude?: string[] }) {
  ...
  return (
    <>
      <nav
        className="fixed inset-x-0 bottom-0 z-40 flex border-t border-white/[0.07] bg-surface-1/95 shadow-[0_-14px_40px_rgba(0,0,0,.3)] backdrop-blur-xl md:hidden"
      ...
      >
        {primary.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              clsx(
                'flex flex-1 flex-col items-center justify-center gap-1.5 px-2 py-2.5 text-[0.6875rem] font-semibold leading-none transition-colors',
