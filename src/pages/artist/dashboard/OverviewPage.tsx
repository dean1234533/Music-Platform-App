...
export function OverviewPage() {
  ...
  const isDancer = artist.creatorType === 'dancer'
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        {isDancer ? null : <StatCard label="Tracks" value={formatCount(tracks.length)} />}
        {isDancer ? null : <StatCard label="Preview plays" value={formatCount(totalPreviewPlays)} />}
      </div>

      <div className="rounded-xl border border-surface-border bg-surface-1 px-4 py-3 text-sm text-ink-1">
        {isDancer
          ? 'See the Revenue tab for subscription income and payouts.'
      ...
      {isDancer ? (
        <EmptyState
          ...
          description="Post a link to your YouTube dance video from Community so fans can watch and support you."
          action={
            <Link to="/dashboard/artist/community" className="text-sm font-medium text-brand-400 hover:underline">
              Go to Community →
            </Link>
          }
        />
