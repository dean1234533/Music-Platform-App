...
import { TOOLS, getToolBySlug, type ToolMeta } from '@/content/tools'

const CATEGORY_LABEL: Record<'artist' | 'dj', string> = { artist: 'For artists', dj: 'For DJs' }

const toolLinks = TOOLS.filter((tool): tool is ToolMeta & { category: 'artist' | 'dj' } => tool.category !== null)
...
export function ToolsHubPage() {
  ...
  const [category, setCategory] = useState<'all' | 'artist' | 'dj'>('all')
  ...
  return <ToolShell eyebrow={hub.eyebrow} title={hub.title} description={hub.description} path="/tools">
      ...
      <>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {visible.map((tool, index) => (
            <Link key={tool.slug} to={`/tools/${tool.slug}`} className="group rounded-2xl border border-white/[0.08] bg-white/[0.025] p-6 transition hover:-translate-y-1 hover:border-brand-400/40">
...
function QuickTool({ slug, fields, makeResult }: { slug: string; fields: { label: string; placeholder: string }[]; makeResult: (values: string[]) => string }) {
  ...
  const [result, setResult] = useState('')
  return <ToolShell eyebrow={meta.eyebrow} title={meta.title} description={meta.description} path={`/tools/${slug}`}><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={(event) => { event.preventDefault(); setResult(makeResult(values)) }} className="space-y-5 rounded-2xl border border-white/[0.08] p-6">{fields.map((field, index) => <Field key={field.label} label={field.label} placeholder={field.placeholder} value={values[index]} onChange={(value) => setValues((current) => current.map((item, i) => i === index ? value : item))} />)}<button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-surface-0 hover:bg-brand-400">Show result</button></form>{result ? <OutputCard value={result} onCopy={() => void navigator.clipboard?.writeText(result)} /> : <div className="flex items-center rounded-2xl border border-dashed border-white/10 p-8 text-sm leading-6 text-ink-2">Enter a few details and your result will appear here.</div>}</div></ToolShell>
}
...
export function DjNameGeneratorPage() {
  ...
  const meta = getToolBySlug('dj-name-generator')!
  return <ToolShell eyebrow={meta.eyebrow} title={meta.title} description={meta.description} path="/tools/dj-name-generator"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={generate} className="space-y-5 rounded-2xl border border-white/[0.08] p-6"><Field label="Sound or genre" value={sound} onChange={setSound} placeholder="e.g. house" /><Field label="Mood" value={mood} onChange={setMood} placeholder="e.g. dark" /><Field label="Location (optional)" value={location} onChange={setLocation} placeholder="e.g. Bristol" /><button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-surface-0 hover:bg-brand-400">Find names</button></form>{result ? <OutputCard value={result} onCopy={() => void navigator.clipboard?.writeText(result)} /> : <div className="flex items-center rounded-2xl border border-dashed border-white/10 p-8 text-sm leading-6 text-ink-2">Even with nothing filled in, you'll get four usable options.</div>}</div></ToolShell>
}
...
export function SongTitleGeneratorPage() {
  ...
  const meta = getToolBySlug('song-title-generator')!
  return <ToolShell eyebrow={meta.eyebrow} title={meta.title} description={meta.description} path="/tools/song-title-generator"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={generate} className="space-y-5 rounded-2xl border border-white/[0.08] p-6"><Field label="Theme or story" value={theme} onChange={setTheme} placeholder="e.g. heartbreak" /><Field label="Genre" value={genre} onChange={setGenre} placeholder="e.g. house" /><Field label="Mood" value={mood} onChange={setMood} placeholder="e.g. euphoric" /><button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-surface-0 hover:bg-brand-400">Find titles</button></form>{result ? <OutputCard value={result} onCopy={() => void navigator.clipboard?.writeText(result)} /> : <div className="flex items-center rounded-2xl border border-dashed border-white/10 p-8 text-sm leading-6 text-ink-2">Add a theme and you'll get six real title directions, not just your words shuffled around.</div>}</div></ToolShell>
}

export function RoyaltyCalculatorPage() { return <QuickTool slug="royalty-calculator" fields={[{ label: 'Monthly streams', placeholder: 'e.g. 50000' }, { label: 'Estimated rate per stream (£)', placeholder: 'e.g. 0.003' }, { label: 'Your ownership percentage', placeholder: 'e.g. 100' }]} makeResult={([streams, rate, share]) => { const total = (Number(streams) || 0) * (Number(rate) || 0); const owned = total * ((Number(share) || 100) / 100); return `Estimated gross: £${total.toFixed(2)}\nEstimated artist share: £${owned.toFixed(2)}\n\nThis excludes taxes, distributor fees, label splits and platform-specific adjustments.` }} /> }
...
export function BpmKeyFinderPage() {
  ...
  const meta = getToolBySlug('bpm-key-finder')!
  return <ToolShell eyebrow={meta.eyebrow} title={meta.title} description={meta.description} path="/tools/bpm-key-finder"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={generate} className="space-y-5 rounded-2xl border border-white/[0.08] p-6"><Field label="Track BPM" value={bpm} onChange={setBpm} placeholder="e.g. 126" /><Field label="Musical key (e.g. 8A or C minor)" value={key} onChange={setKey} placeholder="e.g. C minor" /><Field label="Where it sits in the set" value={position} onChange={setPosition} placeholder="e.g. warm-up, peak-time, closing" /><button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-surface-0 hover:bg-brand-400">Get mixing notes</button></form>{result ? <OutputCard value={result} onCopy={() => void navigator.clipboard?.writeText(result)} /> : <div className="flex items-center rounded-2xl border border-dashed border-white/10 p-8 text-sm leading-6 text-ink-2">Add a BPM and key and you'll get real compatible tempos and keys, worked out properly.</div>}</div></ToolShell>
}

export function PlaylistPitchTemplatePage() { return <QuickTool slug="playlist-pitch-template" fields={[{ label: 'Artist and track name', placeholder: 'e.g. Northside Static — Afterimage' }, { label: 'Genre and mood', placeholder: 'e.g. electronic soul, late-night' }, { label: 'Release date', placeholder: 'e.g. 12 October 2026' }, { label: 'One sentence about the track', placeholder: 'e.g. Built for the drive home after a long night' }]} makeResult={([track, genre, date, story]) => `Subject: ${track || 'New release'} — playlist consideration\n\nHi,\n\nI’m sharing ${track || 'a new track'}, a ${genre || 'genre-blending'} release arriving ${date || 'soon'}. ${story || 'It is built for listeners looking for something fresh and emotionally direct.'}\n\nThanks for listening,\n${track || 'The artist'}`} /> }
export function SocialCaptionGeneratorPage() { return <QuickTool slug="social-caption-generator" fields={[{ label: 'Track (or artist) name', placeholder: 'e.g. Afterimage' }, { label: 'One line describing the sound or vibe', placeholder: 'e.g. Made for late nights and loud headphones' }, { label: 'What you want people to do', placeholder: 'e.g. link in bio, save it, share with a friend' }]} makeResult={([track, moment, call]) => [`${track || 'The new release'} is out. ${moment || 'Made for late nights and loud headphones.'} Listen now — ${call || 'link in bio.'}`, `A little look behind ${track || 'the track'}. ${moment || 'Every layer has a story.'} ${call || 'Tell us what you hear first.'}`, `${track || 'New music'} is yours now. Thanks for being here while it took shape. ${call || 'Press play and share it with someone who needs it.'}`].join('\n\n')} /> }
export function DjSetlistPlannerPage() { return <QuickTool slug="dj-setlist-planner" fields={[{ label: 'Set length in minutes', placeholder: 'e.g. 60' }, { label: 'Starting energy (1–10)', placeholder: 'e.g. 4' }, { label: 'Closing energy (1–10)', placeholder: 'e.g. 8' }]} makeResult={([length, start, end]) => { const minutes = Number(length) || 60; const opening = Number(start) || 4; const closing = Number(end) || 8; return `Set arc — ${minutes} minutes\n\n0–${Math.round(minutes * .2)} min: Warm-up and establish the room (energy ${opening}/10).\n${Math.round(minutes * .2)}–${Math.round(minutes * .65)} min: Build tension, introduce your signature selections.\n${Math.round(minutes * .65)}–${Math.round(minutes * .85)} min: Peak-time run (energy ${Math.max(opening, closing - 1)}/10).\n${Math.round(minutes * .85)}–${minutes} min: Close with intention (energy ${closing}/10).\n\nLeave space for the room—this is a map, not a script.` }} /> }
...
export function MusicGenreGuidePage() {
  ...
  const meta = getToolBySlug('music-genre-guide')!
  return <ToolShell eyebrow={meta.eyebrow} title={meta.title} description={meta.description} path="/tools/music-genre-guide"><div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={generate} className="space-y-5 rounded-2xl border border-white/[0.08] p-6"><Field label="Genre" value={sound} onChange={setSound} placeholder="e.g. house, drum and bass, lo-fi" /><button type="submit" className="w-full rounded-full bg-brand-500 px-5 py-3 text-sm font-semibold text-surface-0 hover:bg-brand-400">Look it up</button></form>{result ? <OutputCard value={result} onCopy={() => void navigator.clipboard?.writeText(result)} /> : <div className="flex items-center rounded-2xl border border-dashed border-white/10 p-8 text-sm leading-6 text-ink-2">Covers 19 common genres — house, techno, DnB, hip hop, folk and more.</div>}</div></ToolShell>
}
