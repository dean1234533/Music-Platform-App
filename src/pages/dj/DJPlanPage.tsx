...
export function DJPlanPage() {
  ...
  useEffect(() => {
    void listActiveSubscriptionPlansForRole('dj').then(setPlans)
  }, [])

  useEffect(() => {
    if (!firebaseUser) return
    return subscribeToOwnSubscription(firebaseUser.uid, 'dj', setSubscription)
  }, [firebaseUser])
  ...
  async function handleSubscribe(planId: string) {
    ...
    try {
      await subscribeToPlan(planId, 'dj')
    } finally {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-2">Free gives you the discovery workflow — upgrade for unlimited requests, filtering, and crates.</p>
      </div>
      ...
      {plans === null ? (
        ...
        <EmptyState
          ...
          title="No DJ plans configured yet"
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {plans.map((plan) => {
            ...
            return (
              <div key={plan.planId} className={`rounded-2xl border p-6 ${plan.recommended ? 'border-dj-500/50 bg-dj-500/5' : 'border-surface-border bg-surface-1'}`}>
